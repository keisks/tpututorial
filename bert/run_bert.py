...
flags.DEFINE_bool(
    "wikihow", False,
    "Use wikihow (which results in just a single ctx field rather than ctx_a and ctx_b"
)
...
def _part_a(item):
    ...
    if 'ctx' not in item:
        return item['ctx_a']
    if FLAGS.wikihow or len(item['ctx']) == len(item['ctx_a']):
        return item['ctx']
    return item['ctx_a']
